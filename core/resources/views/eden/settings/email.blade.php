    ...
    <form action="{{ route('admin.settings.email.update') }}" method="post" class="dash-form">
      ...
      <div style="display: flex; flex-direction: column; gap: 20px;">
        <div style="display: grid; grid-template-columns: minmax(0, 260px) minmax(0, 1fr); gap: 16px; align-items: start;">
          <div>
            ...
            <select id="email_method" name="email_method" class="dash-input">
              <option value="php" @if($method === 'php') selected @endif>PHP mail</option>
              <option value="smtp" @if($method === 'smtp') selected @endif>SMTP</option>
              <option value="sendgrid" @if($method === 'sendgrid') selected @endif>SendGrid API</option>
              <option value="mailjet" @if($method === 'mailjet') selected @endif>Mailjet API</option>
            </select>
          ...
          <div id="email-method-smtp" style="@if($method === 'smtp') @else display:none; @endif">
        ...
        <div>
          ...
          <p class="dash-hint" style="margin-bottom:12px;">This HTML wrapper is used for all emails.</p>
          ...
          <div style="margin-top:14px;">
            ...
            @php
              $templateValue = old('email_template', $emailTemplate);
            @endphp
            <textarea id="email_template" name="email_template" rows="10" class="dash-input" style="font-family: ui-monospace, monospace; font-size: 0.85rem;" placeholder="HTML template...">{{ $templateValue }}</textarea>
        ...
        <div>
          ...
          <div style="display:flex;flex-direction:column;gap:16px;">
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
              <input type="checkbox" name="email_notifications_enabled" value="1" @if(old('email_notifications_enabled', $emailNotificationsEnabled)) checked @endif>
              <span class="dash-label" style="margin-bottom:0;">Enable email notifications</span>
            </label>
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
              <input type="checkbox" name="welcome_email_enabled" value="1" @if(old('welcome_email_enabled', $welcomeEmailEnabled)) checked @endif>
              <span class="dash-label" style="margin-bottom:0;">Send welcome email on signup</span>
            </label>
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
              <input type="checkbox" name="verification_required" value="1" @if(old('verification_required', $verificationRequired)) checked @endif>
          ...
          <div style="margin-top:18px;display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:20px;">
            <div>
              ...
              @php
                $wSubject = old('welcome_subject') ?? ($welcomeTemplate->subject ?? 'Welcome to Eden');
                $wBody = old('welcome_body') ?? ($welcomeTemplate->email_body ?? '');
              @endphp
              <div style="display:flex;flex-direction:column;gap:10px;">
                <div>
                  <label for="welcome_subject" class="dash-label">Subject</label>
                  <input type="text" id="welcome_subject" name="welcome_subject" class="dash-input" value="{{ $wSubject }}">
                  @error('welcome_subject') <span class="dash-error">{{ $message }}</span> @enderror
                </div>
                <div>
                  <label for="welcome_body" class="dash-label">HTML body</label>
                  <textarea id="welcome_body" name="welcome_body" rows="8" class="dash-input" style="font-family: ui-monospace, monospace; font-size: 0.85rem;">{{ $wBody }}</textarea>
            ...
            <div>
              ...
              @php
                $vSubject = old('verification_subject') ?? ($verificationTemplate->subject ?? 'Verify your email');
                $vBody = old('verification_body') ?? ($verificationTemplate->email_body ?? '');
              @endphp
              <div style="display:flex;flex-direction:column;gap:10px;">
                <div>
                  <label for="verification_subject" class="dash-label">Subject</label>
                  <input type="text" id="verification_subject" name="verification_subject" class="dash-input" value="{{ $vSubject }}">
                ...
                <div>
                  ...
                  <textarea id="verification_body" name="verification_body" rows="8" class="dash-input" style="font-family: ui-monospace, monospace; font-size: 0.85rem;">{{ $vBody }}</textarea>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (methodSelect && smtpBlock) {
      methodSelect.addEventListener('change', function() {
        smtpBlock.style.display = this.value === 'smtp' ? '' : 'none';
      });
    }
  });
</script>
