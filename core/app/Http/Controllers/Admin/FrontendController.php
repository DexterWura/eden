...
class FrontendController extends Controller
{
    ...
    public function frontendSections($key)
    {
        ...
        if (!$content && str_starts_with($key, 'marketplace_')) {
    ...
    public function frontendContent(Request $request, $key)
    {
        ...
        if ($request->id) {
            ...
            if (!$content && $type == 'content' && str_starts_with($key, 'marketplace_')) {
        ...
        if ($type == 'content' && str_starts_with($key, 'marketplace_')) {
            $existing = is_string($content->data_values) ? json_decode($content->data_values, true) : (array) $content->data_values;
            $inputContentValue = array_merge($existing ?? [], $inputContentValue);
            if ($key === 'marketplace_hero') {
                unset($inputContentValue['subheading'], $inputContentValue['hero_status'], $inputContentValue['status']);
            } elseif (isset($inputContentValue['hero_status'])) {
                $inputContentValue['status'] = $inputContentValue['hero_status'];
            }
        }
