...
class AutomaticGatewayController extends Controller
{
    ...
    public function edit($alias)
    {
        ...
        if ($gateway->currencies->count()) {
            ...
            // Mask sensitive gateway parameters if demo user
            if ($this->shouldMaskData() && $globalParameters) {
                $globalParameters = (object) $this->maskForDemo((array) $globalParameters, [
                    'api_key', 'secret_key', 'private_key', 'public_key', 'access_token',
                    'client_id', 'client_secret', 'merchant_id', 'merchant_key'
                ]);
            }
        }
        
        // Mask global parameters if demo user
        if ($this->shouldMaskData() && $parameters) {
            $parameters = $parameters->map(function ($param, $key) {
                if (isset($param->value) && !empty($param->value)) {
                    $sensitiveKeys = ['api_key', 'secret_key', 'private_key', 'public_key', 'access_token', 'client_id', 'client_secret'];
                    if (in_array(strtolower($key), $sensitiveKeys) || 
                        in_array(strtolower($param->title ?? ''), ['api key', 'secret key', 'private key', 'public key', 'access token', 'client id', 'client secret'])) {
                        $param->value = '{protected in demo mode}';
                    }
                }
                return $param;
            });
