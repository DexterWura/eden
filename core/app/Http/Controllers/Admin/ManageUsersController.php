...
class ManageUsersController extends Controller
{
    ...
    protected function userData($scope = null)
    {
        ...
        $users = $users->searchable(['username', 'email'])->orderBy('id', 'desc')->paginate(getPaginate());
        
        // Mask sensitive data if demo user
        if ($this->shouldMaskData()) {
            $users->getCollection()->transform(function ($user) {
                $user->email = $this->maskEmail($user->email);
                $user->mobile = $this->maskField($user->mobile);
                return $user;
            });
        }
    ...
    public function kycApprove($id)
    {
        ...
        admin_audit_log('user.kyc.approved', "KYC approved for user: {$user->username}", $user);
    ...
    public function kycReject(Request $request, $id)
    {
        ...
        admin_audit_log('user.kyc.rejected', "KYC rejected for user: {$user->username}. Reason: {$request->reason}", $user, [], ['reason' => $request->reason]);
    ...
    public function update(Request $request, $id)
    {
        ...
        $hasStates = countryHasStates($countryCode);
        ...
        $request->validate($validationRules, [
            'state.required' => 'State is required for the selected country',
            'zip.required' => 'Zip/Postal code is required for the selected country',
        ...
        admin_audit_log('user.updated', "User updated: {$user->username}", $user);
    ...
    public function addSubBalance(Request $request, $id)
    {
        ...
        admin_audit_log('user.balance.' . $request->act, "Balance {$request->act} for user: {$user->username}. Amount: {$amount}. Remark: {$request->remark}", $user, [], ['amount' => $amount, 'act' => $request->act, 'remark' => $request->remark]);
    ...
    public function status(Request $request, $id)
    {
        ...
        admin_audit_log(
            $user->status == Status::USER_ACTIVE ? 'user.unbanned' : 'user.banned',
            ($user->status == Status::USER_ACTIVE ? 'User unbanned: ' : 'User banned: ') . $user->username,
            $user,
            [],
            ['status' => $user->status, 'ban_reason' => $user->ban_reason ?? null]
        );

        return back()->withNotify($notify);
    }

    public function toggleTestUser(Request $request, $id)
    {
        $user = User::findOrFail($id);
        
        // Check if trying to mark as test user
        if (!$user->is_test_user) {
            // Count current test users (excluding this user)
            $currentTestUserCount = User::where('is_test_user', true)->where('id', '!=', $user->id)->count();
            
            if ($currentTestUserCount >= 2) {
                $notify[] = ['error', 'Maximum of 2 test users allowed. Please unmark another test user first.'];
                return back()->withNotify($notify);
            }
            
            $user->is_test_user = true;
            $action = 'marked as test user';
            $notify[] = ['success', 'User marked as test user successfully'];
        } else {
            $user->is_test_user = false;
            $action = 'unmarked as test user';
            $notify[] = ['success', 'User unmarked as test user successfully'];
        }
        
        $user->save();
        
        admin_audit_log(
            'user.test_user.toggle',
