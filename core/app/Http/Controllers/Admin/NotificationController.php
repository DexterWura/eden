...
class NotificationController extends Controller
{
    ...
    public function emailSetting()
    {
        ...
        // Mask sensitive email config if demo user
        $mailConfig = gs('mail_config');
        if (auth('admin')->user() && auth('admin')->user()->isDemoUser() && $mailConfig) {
            if (isset($mailConfig->password)) {
                $mailConfig->password = '{protected in demo mode}';
            }
            if (isset($mailConfig->appkey)) {
                $mailConfig->appkey = '{protected in demo mode}';
            }
            if (isset($mailConfig->secret_key)) {
                $mailConfig->secret_key = '{protected in demo mode}';
            }
            if (isset($mailConfig->public_key)) {
                $mailConfig->public_key = '{protected in demo mode}';
            }
        }
    ...
    public  function smsSetting()
    {
        ...
        // Mask sensitive SMS config if demo user
        $smsConfig = gs('sms_config');
        if (auth('admin')->user() && auth('admin')->user()->isDemoUser() && $smsConfig) {
            // Mask all API keys and secrets in SMS config
            foreach ($smsConfig as $provider => $config) {
                if (is_object($config)) {
                    foreach ($config as $key => $value) {
                        if (in_array(strtolower($key), ['api_key', 'api_secret', 'secret', 'password', 'apiv2_key', 'username']) && !empty($value)) {
                            $smsConfig->$provider->$key = '{protected in demo mode}';
                        }
                    }
                }
            }
    ...
    public function pushSetting()
    {
        ...
        // Mask sensitive Firebase config if demo user
        $firebaseConfig = gs('firebase_config');
        if (auth('admin')->user() && auth('admin')->user()->isDemoUser() && $firebaseConfig) {
            if (isset($firebaseConfig->apiKey)) {
                $firebaseConfig->apiKey = '{protected in demo mode}';
            }
            if (isset($firebaseConfig->messagingSenderId)) {
                $firebaseConfig->messagingSenderId = '{protected in demo mode}';
            }
            if (isset($firebaseConfig->appId)) {
                $firebaseConfig->appId = '{protected in demo mode}';
            }
        }
