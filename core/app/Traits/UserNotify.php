...
trait UserNotify
{
    public static function notifyToUser(){
        return [
            ...
            'madeSaleUsers'         => 'Users who have made any sale',
            'activeListingUsers'    => 'Users who have active listings',
            'neverListedUsers'      => 'Users who have never listed anything',
            'neverBoughtUsers'      => 'Users who have never bought anything',
            'hasBoughtUsers'        => 'Users who have made at least one purchase',
            'hasListedUsers'        => 'Users who have ever listed (any status)',
            'listedButNoActiveUsers' => 'Users who have listed but have no active listings',
            'hasWinningBidUsers'    => 'Users who have won at least one bid',
            'hasWatchlistUsers'     => 'Users with items in watchlist',
        ];
    }
    ...
    public function scopeMadeSaleUsers($query)
    {
        return $query->whereHas('soldListings');
    }

    public function scopeActiveListingUsers($query)
    {
        return $query->whereHas('activeListings');
    }

    public function scopeNeverListedUsers($query)
    {
        return $query->whereDoesntHave('listings');
    }

    public function scopeNeverBoughtUsers($query)
    {
        return $query->whereDoesntHave('escrowsAsBuyer', function ($q) {
            $q->where('status', Status::ESCROW_COMPLETED);
        });
    }

    public function scopeHasBoughtUsers($query)
    {
        return $query->whereHas('escrowsAsBuyer', function ($q) {
            $q->where('status', Status::ESCROW_COMPLETED);
        });
    }

    public function scopeHasListedUsers($query)
    {
        return $query->whereHas('listings');
    }

    public function scopeListedButNoActiveUsers($query)
    {
        return $query->whereHas('listings')->whereDoesntHave('activeListings');
    }

    public function scopeHasWinningBidUsers($query)
    {
        return $query->whereHas('winningBids');
    }

    public function scopeHasWatchlistUsers($query)
    {
        return $query->whereHas('watchlist');
    }
