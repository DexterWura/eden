...
class FileManager
{
    ...
    */
	protected function uploadImage(){
        ...
        //save WebP version for better performance (25-35% smaller)
	    $this->saveWebpVersion($image);
	    ...
	    if ($this->thumb) {
	        ...
	        $this->saveWebpVersion($thumbImage, 'thumb_');
	    }
	}

    /**
     * Save a WebP version of the image alongside the original
     *
     * @param \Intervention\Image\Interfaces\ImageInterface $image
     * @param string $prefix Optional prefix (e.g. thumb_)
     * @return void
     */
	protected function saveWebpVersion($image, string $prefix = ''): void
    {
        try {
            $webpFilename = $prefix . preg_replace('/\.(jpg|jpeg|png)$/i', '.webp', $this->filename);
            $webpPath = $this->path . '/' . $webpFilename;
            $image->save($webpPath, 80);
        } catch (\Throwable $e) {
            // WebP not supported or save failed; continue without WebP
        }
    }
    ...
    */
	public function removeFile($path = null)
	{
	    ...
	    file_exists($webpPath) && is_file($webpPath) ? @unlink($webpPath) : false;

	    if ($this->thumb) {
	    	$thumbPath = $this->path . '/thumb_' . $this->old;
	    	file_exists($thumbPath) && is_file($thumbPath) ? @unlink($thumbPath) : false;
	    	$thumbWebp = preg_replace('/\.(jpg|jpeg|png)$/i', '.webp', $thumbPath);
	    	file_exists($thumbWebp) && is_file($thumbWebp) ? @unlink($thumbWebp) : false;
	    }
	}
